...
export const DetailTop = ({ course }) => {
  const { title, code, section, dateRange, days, links } = course;
  const { startDate, endDate } = dateRange;
  const { courseInfo, blackboard } = links;
  ...
  div.innerHTML = `
    <div style="display: flex; font-size: 15px; gap: 8px; padding-bottom: 20px;">
      ...
      <p>Section ${section}</p>
      <a href="${blackboard}" style="border-left: 2px solid black; padding-left: 8px; color: #2B7DA1;">Blackboard</a>
    </div>
